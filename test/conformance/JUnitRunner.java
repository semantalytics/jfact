...
//					System.out.println(t);
		...
		switch (t) {
			case CONSISTENCY: {
				...
				if (!consistent) {
					Assert.assertEquals(
							b.toString()
									+ logTroubles(premiseOntology,true, consistent, null, t),
							true, consistent);
				}
			}
				break;
			case INCONSISTENCY: {
				...
				if (consistent) {
					Assert.assertEquals(
							b.toString()
									+ logTroubles(premiseOntology,false, consistent, null, t),
							false, consistent);
				}
			}
				break;
	...
	public String logTroubles(OWLOntology o, boolean expected, boolean actual, OWLAxiom ax,
			TestClasses c) {
		StringBuilder b = new StringBuilder();
