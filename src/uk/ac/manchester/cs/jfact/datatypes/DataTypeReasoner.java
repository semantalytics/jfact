...
public final class DataTypeReasoner {
	...
	private <R extends Comparable<R>> DataTypeSituation<R> getType(DatatypeExpression<R> p) {
		return getType(p.getHostType());
	}
	...
	public boolean addDataEntry(boolean positive, DLVertex v, NamedEntry entry, DepSet dep) {
		switch (v.getType()) {
			case dtDataType: {
			...
			case dtDataExpr:
				return dataExpression(positive, ((DatatypeEntry) entry).getDatatype()
						.asExpression(), dep);
			case dtAnd:
				return false;
			default:
	...
	public <R extends Comparable<R>> boolean dataExpression(boolean positive,
			DatatypeExpression<R> c, DepSet dep) {
		if (c.getKnownFacetValues().isEmpty()) {
			return false;
		}
		if (positive) {
			getType(c.getHostType()).setPType(dep);
		}
		options.getLog().printTemplate(Templates.INTERVAL, (positive ? "+" : "-"), c, "",
				"", "");
		return getType(c).addInterval(positive, c, dep);
	}

	public <R extends Comparable<R>> boolean dataValue(boolean positive, Literal<R> c1,
			DepSet dep) {
		Datatype<R> d = c1.getDatatypeExpression();
		if (positive) {
			getType(d).setPType(dep);
		}
		...
		options.getLog().printTemplate(Templates.INTERVAL, (positive ? "+" : "-"),
				interval, "", "", "");
		return getType(d).addInterval(positive, interval, dep);
	}
		...
		if (size > 1) {
			...
			for (int i = 0; i < size; i++) {
				...
				for (int j = i + 1; j < size; j++) {
					...
					if ((ds1.getType().isSubType(ds2.getType()) && ds1.hasPType() && ds2
							.hasNType())
							|| (ds2.getType().isSubType(ds1.getType()) && ds2.hasPType() && ds1
									.hasNType())) {
