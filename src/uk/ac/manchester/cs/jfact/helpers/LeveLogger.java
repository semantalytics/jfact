...
public final class LeveLogger {
	public enum Templates {
		TAX_TRYING("\nTAX: trying '%s' [= '%s'... "), INTERVAL(" %s%s"), CLASH(" DT-%s"), CHECKCLASH(" DT-TT"), CREATE_EDGE(" ce(%s%s%s,%s)"), IS_BLOCKED_FAILURE_BY(" fb(%s,%s)"), LOG_NODE_BLOCKED(" %sb(%s%s%s)"), LOG_SR_NODE(" %s(%s[%s],%s)"), DETERMINE_SORTS(
				"\nThere are %s different sorts in TBox\n"), SET_ORDER_DEFAULTS1("orSortSat: initial=%s, default=%s"), SET_ORDER_DEFAULTS2(", used=%s\n orSortSub: initial=%s, default=%s"), SET_ORDER_DEFAULTS3(", used=%s\n"), WRITE_STATE(
				...
				"\nDAG entry %s is unsatisfiable\n"), CAN_BE_CACHED(" cf(%s)"), CHECK_MERGE_CLASH(" x(%s,%s%s)"), COMMON_TACTIC_BODY_OR(" E(%s)"), COMMON_TACTIC_BODY_SOME(" nf(%s)"), COMMON_TACTIC_BODY_SOME2(" f(%s):"), CONSISTENT_NOMINAL("\nThe ontology is %s"), DN(" DN(%s%s)"), CN(
				" cn(%s%s)"), NN(" NN(%s)"), E(" E(%s,%s,%s)"), LOG_FINISH_ENTRY(" Clash%s"), SPACE(" %s"), DLVERTEXPrint("[d(%s/%s),s(%s/%s),b(%s/%s),g(%s/%s),f(%s/%s)] %s"), DLVERTEXPrint2("(%s) %s %s"), DLVERTEXPrint3(" %s{%s} %s"), DLVERTEXPrint4(" %s, %s => %s"), LOGCACHEENTRY(
				"\nConst cache: element %s"), DLCOMPLETIONTREEARC("<%s%s>"), DLCONCEPTTAXONOMY("Totally %s subsumption tests was made\nAmong them %s (%s) successfull\n" + "Besides that %s successfull and %s unsuccessfull subsumption tests were cached\n"
				+ "%sThere were made %s search calls\n" + "There were made %s Sub calls, of which %s non-trivial\nCurrent efficiency (wrt Brute-force) is %s\n"), PRINTDAGUSAGE("There are %s unused DAG entries (% of %s total)\n"), READCONFIG(
